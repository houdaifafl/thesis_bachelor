...
The frontend was developed first. Development began with a visual mockup of the login page, given to the agent together with an instruction to reproduce it as closely as possible; the agent was told in advance to ask clarifying questions before generating any code, and additional details not fully captured by the mockup itself --- specific colors, fonts, and layout choices --- were provided before generation started rather than left for the agent to infer. Styling was explicitly constrained to Bootstrap, backend integration was deliberately postponed so the page could first be built as a UI-only component, and non-essential elements such as a "Forgot password" link were left as placeholders. The agent was also not permitted to modify any existing file without asking first. \Cref{lst:login-mockup-followup} shows the additional details provided in response to the agent's clarifying questions, supplying exactly this kind of information before generation began (\Cref{fig:login-mockup}).

\begin{listing}[H]
\begin{center}
\begin{promptbox}
1- Styling approach: for this project, use Bootstrap as the primary styling approach. All components should leverage Bootstrap's built-in classes for layout, responsiveness, and general styling. No need for CSS Modules or styled-components unless custom, non-Bootstrap styles are required.

2- Routing: Yes, we are using React Router for routing in this project. The "Sign Up" link should navigate to the /signup route, and the login route (or a similar authenticated route) should navigate to the /MainPage route after successful authentication. But know i want to start small with the login page.

3- API integration: Now just the UI.

4- Forgot password: now just keep it a placeholder UI element.

5- Color details:
- Background: E9E3E3
\end{promptbox}
\end{center}
\caption{Follow-up details provided in response to the agent's clarifying questions on the login page mockup.}
\label{lst:login-mockup-followup}
\end{listing}
...
The first generated version matched the mockup closely in color and overall structure, but the login card was too narrow and sat too centered on the page, which did not match the intended design.

This was corrected through further prompts adjusting the card's width and the alignment of the input fields --- an iterative process in which the first output served as a starting point rather than a finished result.
