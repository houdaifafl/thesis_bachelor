...
The workspace ID in the URL should represent the active workspace being displayed.

Frontend requirements:
- Replace the generic /main route with a workspace-specific route.
- After login, redirect the user to their default workspace route.
- When the route is /workspace/:workspaceId, load the data for that workspace only.
- If the user switches workspace from the dropdown, update the URL to the selected workspace ID.
- The header should still display the active workspace name.
- Do not break the existing workspace dropdown, collection sidebar, or overview tab behavior.
\end{promptbox}
\end{center}
\caption{Excerpt of a precisely specified prompt for the workspace-scoped routing feature, from Part 1 of this thesis's practical project.}
\label{lst:workspace-routing-prompt}
\end{listing}
